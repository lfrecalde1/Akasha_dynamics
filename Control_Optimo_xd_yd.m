...
%   psid = atan2(ydp,xdp);
  ...
  %Circunferencia
%   xd = 1.5*cos(0.05*t)+1.75;    xdp = -1.5*0.05*sin(0.05*t);      
%   yd = 1.5*sin(0.05*t)+1.75;    ydp =  1.5*0.05*cos(0.05*t);      
%   psid = atan2(ydp,xdp);
...
for k=1:length(t)-1
    ...
    f_obj = @(z) L_ope_xd_yd(z,Q,hd,hd1,h,psi(k),psid(k),a,dt);
